...
namespace delaunator {
    ...
    inline static size_t fast_mod(const size_t i, const size_t c) {
        return i >= c ? i % c : i;
    }

    // Kahan and Babuska summation, Neumaier variant; accumulates less FP error
    inline static double sum(const std::vector<double>& x) {
        double sum = x[0];
        double err = 0.0;
    ...
    inline static double circumradius(const Point& p1, const Point& p2, const Point& p3)
    {
    ...
    inline static bool clockwise(const Point& p0, const Point& p1, const Point& p2)
    {
    ...
    inline static bool clockwise(double px, double py, double qx, double qy,
        double rx, double ry)
    {
        Point p0(px, py);
        Point p1(qx, qy);
    ...
    inline static bool counterclockwise(const Point& p0, const Point& p1, const Point& p2)
    {
        Point v0 = Point::vector(p0, p1);
        Point v1 = Point::vector(p0, p2);
        double det = Point::determinant(v0, v1);
    ...
    inline static bool counterclockwise(double px, double py, double qx, double qy,
        double rx, double ry)
    {
        Point p0(px, py);
        Point p1(qx, qy);
    ...
    inline static Point circumcenter(
        const double ax,
        const double ay,
        const double bx,
        const double by,
    ...
    inline static bool check_pts_equal(double x1, double y1, double x2, double y2) {
        return std::fabs(x1 - x2) <= EPSILON &&
            std::fabs(y1 - y2) <= EPSILON;
    }

    // monotonically increases with real angle, but doesn't need expensive trigonometry
    inline static double pseudo_angle(const double dx, const double dy) {
        ...
        : coords(in_coords), m_points(in_coords)
    {
        ...
        std::sort(std::execution::par, ids.begin(), ids.end(),
